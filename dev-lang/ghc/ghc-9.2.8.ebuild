...
ghc_setup_cflags() {
	...
	filter-flags -flto -flto=thin -flto=full
	strip-flags
...
src_prepare() {
	...
	if use binary; then
		...
		eapply "${FILESDIR}"/${PN}-9.2.6-no-pie.patch
...
src_configure() {
	# Change llvm target to 'clang -print-target-triple'
	sed -i -e "s|LlvmTarget=.*|LlvmTarget=\"\$\(clang -print-target-triple\)\"|" "${S}/configure"
	# Add -Wno-unused-command-argument to CFLAGS/CXXFLAGS to turn off the warning
	sed -i -e "s|CONF_CC_OPTS_STAGE2=\"--target=\$LlvmTarget.*|CONF_CC_OPTS_STAGE2=\"--target=\$LlvmTarget \$CONF_CC_OPTS_STAGE2 -Wno-unused-command-line-argument\"|" "${S}/configure"
	sed -i -e "s|CONF_CXX_OPTS_STAGE2=\"--target=\$LlvmTarget.*|CONF_CXX_OPTS_STAGE2=\"--target=\$LlvmTarget \$CONF_CXX_OPTS_STAGE2 -Wno-unused-command-line-argument\"|" "${S}/configure"
	if ! use binary; then
		# initialize build.mk
...
src_compile() {
	if ! use binary; then
		...
		if ! is_crosscompile; then
			...
			${MAKE:-make} ${MAKEOPTS} ghc/stage2/build/tmp/ghc-stage2 ${EXTRA_EMAKE} || ${MAKE:-make} ${MAKEOPTS} ghc/stage2/build/tmp/ghc-stage2 ${EXTRA_EMAKE} || die
